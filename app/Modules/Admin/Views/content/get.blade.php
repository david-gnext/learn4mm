     ...
     if($link == "major") {
        foreach($subject as $m) {
         ...
         foreach($subject as $m) {
            ...
            echo "<td><input type='text' name='hint' value='".htmlentities($m->hint)."' class='w3-input readonly' readonly></td>";
            echo '<td><select class="w3-select w3-hover-none w3-col s8" name="isRead"><option value="1"'.($m->isFill == 1 ?"selected":"").'>Fill</option><option value="0" '.($m->isFill ==0 ?"selected":"").'>No Fill</option><option value="2" '.($m->isFill ==2 ?"selected":"").'>Voice</option></select></td>';
